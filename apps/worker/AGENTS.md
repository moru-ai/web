...
- `plugins/external/app.db.ts` decorates `fastify.db.client` with a typed `ConvexHttpClient` so workers can call public Convex mutations; authenticate each call with `CONVEX_WORKER_API_KEY`.
